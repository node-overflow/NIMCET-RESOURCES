...
/* ------------------------------------------------------------------ */
function showPasswordPrompt(reason, updatePassword) {
    ...
    els.pwErrMsg.textContent = incorrect ? 'Incorrect password. Try again.' : '';
    els.pwOverlay.classList.add('show');
    requestAnimationFrame(() => els.pwInput.focus());
}

function submitPassword() {
    if (!pendingPasswordCallback) return;
...
function cancelPasswordPrompt() {
    ...
    if (currentLoadingTask) { try { currentLoadingTask.destroy(); } catch (e) { } }
    setLoading(false);
    els.dropZone.style.display = 'flex';
}
...
function jumpToPage(num, opts = {}) {
    num = Math.max(1, Math.min(numPages, num));
    const m = pageMeta[num - 1];
    if (!m) return;
    m.wrap.scrollIntoView({ behavior: opts.instant ? 'auto' : 'smooth', block: 'start' });
...
els.pageInput.addEventListener('keydown', (e) => {
    if (e.key === 'Enter') { jumpToPage(parseInt(els.pageInput.value, 10) || 1, { instant: true }); els.pageInput.blur(); }
});
els.pageInput.addEventListener('change', () => jumpToPage(parseInt(els.pageInput.value, 10) || 1, { instant: true }));
...
els.scroller.addEventListener('touchstart', (e) => {
    if (!pdfDoc || e.touches.length !== 2) return;
    pinchActive = true;
    pinchRatio = 1;
